...
namespace RazorExercise.Utilities
{
        ...
        public static async Task<string> ProcessFormFile(IFormFile formFile, ModelStateDictionary modelState)
        {
            ...
            if (formFile.ContentType.ToLower() != "text/plain")
            {
                modelState.AddModelError(formFile.Name,
                                         $"El {fieldDisplayName}archivo ({fileName}) debe ser un archivo de texto.");
            }
            ...
            {
                modelState.AddModelError(formFile.Name, $"El {fieldDisplayName}archivo ({fileName}) esta vacío.");
            }
            else if (formFile.Length > 1048576)
            {
                modelState.AddModelError(formFile.Name, $"El {fieldDisplayName}archivo ({fileName}) excede 1 MB.");
            }
            else
            {
                try
                {
                                ...
                                detectEncodingFromByteOrderMarks: true))
                    {
                        ...
                        {
                            modelState.AddModelError(formFile.Name,
                                                     $"El {fieldDisplayName}archivo ({fileName}) esta vacío.");
                        }
                    }
                }
                catch (Exception ex)
                {
                    modelState.AddModelError(formFile.Name,
                                             $"el {fieldDisplayName}archivo ({fileName}) fallo en cargarse. " +
                                             $"Por favor, pónganse en contacto con el servicio de asistencia para obtener ayuda. Error: {ex.Message}");
                    // Log the exception
                }
            }
